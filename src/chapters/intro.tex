...
\begin{enumerate}
    \item To collect and preprocess PM$_{2.5}$ and meteorological data, and construct a wind-aware dynamic graph using distance, PM$_{2.5}$ correlation, wind speed, and wind direction for both monitoring stations and virtual nodes.
    \item To develop and evaluate baseline and GAT-GRU models for PM$_{2.5}$ forecasting, and provide explainability through a dashboard that visualizes forecasts, Air Quality Index (\gls{aqi}), spatial patterns, virtual-node estimates, and model explanations.
\end{enumerate}
